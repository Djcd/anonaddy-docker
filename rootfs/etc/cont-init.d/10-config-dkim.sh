...
echo "generating DNS TXT record with public key and storing it in /data/dkim/dkim.${ANONADDY_DKIM_SELECTOR}.txt"
echo ""
rspamadm dkim_keygen -s "${ANONADDY_DKIM_SELECTOR}" -b 2048 -k "${DKIM_PRIVATE_KEY}" | tee -a "/data/dkim/dkim.${ANONADDY_DKIM_SELECTOR}.txt"
